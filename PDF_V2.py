...
def create_pdf(data):
    class PDF(FPDF):
        def header(self):
            ...
            self.cell(0, 10, f"Relatório de Vendas do Dia {data['Data']}", ln=True, align="C")
    ...
    for index, row in data["Detalhes"].iterrows():
        pdf.set_font("Arial", style="B", size=10)  # Negrito para forma de pagamento
        pdf.cell(0, 10, "Forma de Pagamento: " + row['Forma de Pagamento'], ln=True)
        pdf.set_font("Arial", size=10)  # Volta para normal
